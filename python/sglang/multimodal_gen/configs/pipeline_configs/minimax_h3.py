...
class MiniMaxH3PipelineConfig(PipelineConfig):
    ...
    def _force_vae_resident(self, server_args) -> None:
        ...
        if os.environ.get("MINIMAX_H3_FORCE_VAE_RESIDENT", "1").strip().lower() in (
            ...
            return
        # MPS uses synchronous layerwise offload for every H3 component; the
        # CUDA PCIe decode measurement does not apply to unified memory.
        if current_platform.is_mps():
            return

        require_resident = getattr(server_args, "require_component_resident", None)
        if callable(require_resident):
            for component_name in ("video_vae", "audio_vae"):
                require_resident(
                    component_name,
                    feature_name="MiniMax-H3 VAE decode",
                )
            return
        changed = []

        components = getattr(server_args, "layerwise_offload_components", None)
        if components:
            stripped = [
        ...
        cpu_components = getattr(server_args, "cpu_offload_components", None)
        if cpu_components:
            stripped = [
                name
