...
def _write_legacy_sidecar(
    ...
    *,
    ...
    block_tables = [
        (
            torch.arange(3 * _BLOCK_WIDTH, dtype=torch.float32)
            .reshape(3, _BLOCK_WIDTH)
            .add(index * 1000)
            .bfloat16()
        )
        for index in range(_ARCH.num_layers)
    ...
    final_table = (
        torch.arange(3 * _FINAL_WIDTH, dtype=torch.float32)
...
def test_legacy_sidecar_preserves_flat_timestep_lookup(tmp_path):
    path = tmp_path / "legacy.safetensors"
    timesteps, block_tables, final_table = _write_legacy_sidecar(path)
    cache = MiniMaxH3AdalnCache(
        _ARCH,
        path=str(path),
        model_variant="fl2va",
    ...
    # The old table stores one global row per timestep. The adapter returns a
    # padded row-index vector per step; consumers slice it to the real width.
    slots = cache.resolve_slots([timesteps[:2], timesteps[2:]])
    assert slots.tolist() == [[0, 1], [2, 0]]
    assert cache.lookup(timesteps[[0, 2]]).tolist() == [0, 2]

    blocks = cache.block_all(cache_plan_index=slots[0], num_timesteps=2)
    for index in range(_ARCH.num_layers):
        got = torch.cat(blocks[index], dim=-1).reshape(2, _BLOCK_WIDTH)
        assert torch.equal(got, block_tables[index][:2])
    final = torch.cat(cache.final(slots[0], 2), dim=-1)
    assert torch.equal(final, final_table[:2])
...
def test_legacy_sidecar_requires_matching_checkpoint_fingerprint(tmp_path):
    path = tmp_path / "legacy.safetensors"
    _write_legacy_sidecar(path, fingerprint="sha256:artifact")
    cache = MiniMaxH3AdalnCache(
        _ARCH,
        path=str(path),
        model_variant="fl2va",
        legacy_provenance={
            "format_version": "1",
            "table_layout": "full",
            "source_fingerprint": "sha256:checkpoint",
        },
    )
