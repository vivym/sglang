...
class TestTextEncoderQuantization(unittest.TestCase):
    ...
    def test_legacy_int8_h3_encoder_validates_checkpoint_tensors(self):
        serialized = Int8Config.from_config({"quant_method": "int8", "convrot": True})
        self.get_quant_config.return_value = serialized
        with tempfile.TemporaryDirectory() as tmpdir:
            checkpoint = Path(tmpdir) / "model.safetensors"
            save_file(
                {
        ...
        self.assertIs(model_config.quant_config, serialized)

    def test_legacy_int8_h3_encoder_rejects_missing_scale(self):
        self.get_quant_config.return_value = Int8Config.from_config(
            {"quant_method": "int8", "convrot": True}
        )
        with tempfile.TemporaryDirectory() as tmpdir:
            checkpoint = Path(tmpdir) / "model.safetensors"
            save_file(
            ...
            with self.assertRaisesRegex(
                ComponentCheckpointUnsupportedError, "missing.*weight_scale"
            ):
                _configure_encoder_quantization(
                    model_config,
                    MiniMaxH3Qwen3VLEncoder,
