...
@contextmanager
def _minimax_h3_nsys_capture(enabled: bool):
    """Bracket the complete DiT use site with an exception-safe nsys capture."""
    if not enabled:
        yield
        return
...
class MiniMaxH3DenoisingStage(DenoisingStage):
    ...
    def _maybe_enable_cache_dit(
        ...
        if sampler_mode == "res_multistep":
            ...
            enable_override = getattr(batch.sampling_params, "enable_cache_dit", None)
            cache_requested = (
                super()._cache_dit_requested()
                if enable_override is None
                else bool(enable_override)
            )
            if quality == "high" or cache_requested:
                raise ValueError(
    ...
    def _run_full_loop(self, batch: Req, server_args: ServerArgs) -> None:
        ...
        nsys_requested = os.environ.get("MINIMAX_H3_NSYS_CAPTURE_DIT", "0") == "1"
        if nsys_requested and not current_platform.is_cuda():
            raise RuntimeError("MINIMAX_H3_NSYS_CAPTURE_DIT requires CUDA")
        model = None
        capture_stack = ExitStack()
        capture_stack.enter_context(_minimax_h3_nsys_capture(nsys_requested))
        try:
