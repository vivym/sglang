...
class TransformerQuantLoadSpec:
    ...
    @property
    def is_serialized_legacy_int8(self) -> bool:
        return (
            isinstance(self.quant_config, Int8Config)
            and self.quant_config.is_checkpoint_int8_serialized
        )
...
def _needs_device_weight_postprocess(
    ...
    if quant_name == "int8":
        assert isinstance(quant_config, Int8Config)
        return not quant_config.is_checkpoint_int8_serialized
...
def _resolve_quant_config(
    *,
    ...
    if server_args.quantization is not None:
        ...
        # Online-quant convention: for `fp8`, `mxfp4`, `int8`, and
        # `kitchen_int8`, a
        ...
        quant_kwargs = {}
        if server_args.quantization in {"fp8", "mxfp4", "int8", "kitchen_int8"}:
