...
_ONLINE_ENCODER_QUANTIZATIONS = frozenset({"fp8", "int8", "kitchen_int8", "mxfp4"})
...
def _get_encoder_quant_config(
    ...
    if (
        model_cls is not None
        and model_cls.__name__ == "MiniMaxH3Qwen3VLEncoder"
        and isinstance(quant_config, Int8Config)
        and quant_config.is_checkpoint_int8_serialized
    ):
        if not quant_config.use_convrot:
            raise ComponentCheckpointUnsupportedError(
                "Legacy MiniMax-H3 encoder quant_method='int8' requires convrot=true"
            )
        checkpoint_files = _list_safetensors_files(component_weights_path)
        if not checkpoint_files:
            raise ComponentCheckpointUnsupportedError(
                "Legacy MiniMax-H3 INT8 encoder checkpoint contains no safetensors"
            )
        try:
            inspect_minimax_h3_safetensors(checkpoint_files, legacy_int8=True)
        except ValueError as error:
            raise ComponentCheckpointUnsupportedError(
                f"Invalid legacy MiniMax-H3 INT8 encoder checkpoint: {error}"
            ) from error
    name_mapper = None
    ...
    def load_model(
        ...
        with (
            ...
            if quant_config is not None and not isinstance(quant_config, GGUFConfig):
                ...
                if (
                    isinstance(quant_config, (ComfyNvfp4Config, QuantoInt8Config))
                    or (
                        isinstance(quant_config, KitchenInt8Config)
                        and quant_config.is_checkpoint_int8_serialized
                    )
                    or (
                        isinstance(quant_config, Int8Config)
                        and quant_config.is_checkpoint_int8_serialized
                    )
                ):
