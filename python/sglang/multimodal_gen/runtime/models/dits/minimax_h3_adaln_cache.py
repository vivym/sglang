...
class MiniMaxH3AdalnCache(nn.Module):
    ...
    def _load_legacy_flat(
        self, cache_file, metadata: dict, device: torch.device
    ) -> None:
        """Load the fingerprinted fast-h3 v1 table into the v2 slab interface."""
        expected = self.legacy_provenance
        if expected is None:
            raise ValueError("MiniMax H3 AdaLN v1 cache requires checkpoint provenance")
        for field in ("format_version", "table_layout", "source_fingerprint"):
            expected_value = expected.get(field)
            if not isinstance(expected_value, str) or not expected_value:
                raise ValueError(f"checkpoint AdaLN provenance is missing {field!r}")
            if metadata.get(field) != expected_value:
                raise ValueError(
                    "MiniMax H3 AdaLN cache provenance mismatch for "
                    f"{field}: {metadata.get(field)!r} != {expected_value!r}"
                )
        ...
        required = {
            "timesteps",
            "final_layer",
            *(f"blocks.{index}" for index in range(self.num_layers)),
        }
        actual = set(cache_file.keys())
        if actual != required:
            raise ValueError(
                "MiniMax H3 AdaLN v1 cache tensor set mismatch: "
                f"missing={sorted(required - actual)}, "
                f"unexpected={sorted(actual - required)}"
            )
        timesteps = cache_file.get_tensor("timesteps")
        block_tables = [
            cache_file.get_tensor(f"blocks.{index}") for index in range(self.num_layers)
        ]
        final_table = cache_file.get_tensor("final_layer")
        if (
            timesteps.dtype != _FP32_DTYPE
            or timesteps.ndim != 1
            or timesteps.numel() == 0
            or (
                timesteps.numel() > 1
                and not bool(torch.all(timesteps[1:] > timesteps[:-1]))
            )
        ):
            raise ValueError(
                "MiniMax H3 AdaLN v1 cache timesteps must be non-empty, "
                "strictly increasing float32"
            )
        expected_rows = timesteps.numel()
        for index, table in enumerate(block_tables):
            if table.dtype != _BF16_DTYPE or table.shape != (
                expected_rows,
                self.block_width,
            ):
                raise ValueError(
                    f"MiniMax H3 AdaLN v1 blocks.{index} has invalid shape or dtype"
                )
        if final_table.dtype != _BF16_DTYPE or final_table.shape != (
            expected_rows,
            self.final_width,
        ):
            raise ValueError(
                "MiniMax H3 AdaLN v1 final_layer has invalid shape or dtype"
            )

        plan_timesteps = timesteps[:, None]
        plan_lengths = torch.ones(expected_rows, dtype=torch.int64)
        block_params = torch.stack(block_tables, dim=1)[:, None]
        final_params = final_table[:, None]
    ...
    def resolve_slots(
        ...
        *,
        ...
        if self._legacy_flat:
            ...
            for key in keys:
                step_rows = []
                for timestep_bits in key:
                    slot = self._slots.get((timestep_bits,))
                    if slot is None:
                        raise ValueError(
                            "MiniMax H3 AdaLN cache does not cover the request "
                            "timestep plan"
                        )
                    step_rows.append(slot)
                rows.append(step_rows)
            width = max(map(len, rows))
            padded = [row + [0] * (width - len(row)) for row in rows]
            return torch.tensor(
                padded, dtype=torch.int64, device=self.block_params.device
            )
    ...
    def lookup(self, unique_timesteps: torch.Tensor) -> torch.Tensor:
        if self._legacy_flat:
            ...
            for timestep_bits in key:
                ...
                slots.append(slot)
            return torch.tensor(
                slots, dtype=torch.int64, device=self.block_params.device
            )
